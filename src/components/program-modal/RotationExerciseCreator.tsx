...
import { ExerciseMode, SetType, WorkoutSet, RotationExercise } from '@/types/workout';
import { generateId } from './programModalUtils';

interface Props {
  onSave: (ex: RotationExercise) => void;
  onCancel: () => void;
  existingNames?: string[];
}

export function RotationExerciseCreator({ onSave, onCancel, existingNames = [] }: Props) {
  ...
  // Note: the original CreateProgramModal referenced `existingNames` here without
  // ever passing it as a prop, which would throw a ReferenceError at runtime.
  // We expose it as an optional prop (default []) to make the behaviour explicit.
